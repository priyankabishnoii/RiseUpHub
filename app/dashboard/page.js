...
import {
  Rocket, Home, Map, BookOpen, Clock, Download,
  Settings, LogOut, Plus, ChevronRight, Star,
  Zap, Target, Trophy, Brain, Menu, X, Bell,
  Search, TrendingUp, Award, FileText, BarChart3,
  GraduationCap, Briefcase, Code, Globe, Sparkles,
  ArrowRight, Sun, Moon, CreditCard, History
} from "lucide-react";
...
function Sidebar({ dark, collapsed, setCollapsed, mobileOpen, setMobileOpen }) {
  ...
  const content = (
    <div className={base} style={{ width: collapsed ? 64 : 240 }}>
      {/* Logo */}
      <div className="flex items-center gap-3 px-4 h-16 border-b border-white/[0.06] flex-shrink-0">
        ...
        {!collapsed && (
          <div className="flex flex-col leading-none overflow-hidden">
            ...
            <span className="text-[9px] text-indigo-400 font-semibold tracking-widest uppercase truncate">AI Roadmap</span>
          </div>
        )}
      </div>
      ...
      {/* Bottom nav */}
      <div className={`px-2 py-4 border-t ${dark ? "border-white/[0.06]" : "border-gray-200"} flex flex-col gap-1`}>
        ...
        {/* User avatar */}
        <div className={`flex items-center gap-3 px-3 py-2.5 mt-1 rounded-xl ${dark ? "hover:bg-white/5" : "hover:bg-gray-100"} cursor-pointer transition-all`}>
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              <div className={`text-sm font-semibold truncate ${dark ? "text-white" : "text-gray-900"}`}>Priya</div>
              <div className={`text-xs truncate ${dark ? "text-gray-500" : "text-gray-500"}`}>Free Plan</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          ...
          <p className={`mt-2 text-base ${dark ? "text-gray-400" : "text-gray-600"}`}>
            Ready to build your roadmap today? You have <span className="text-indigo-400 font-bold">2 generations left</span> today.
          </p>
        </div>
        <a
          href="/generate"
      ...
      {USER_STATS.map((s, i) => (
        <motion.div
          ...
          className={`p-5 rounded-2xl border transition-all ${
            dark ? "bg-white/[0.03] border-white/[0.07] hover:border-white/[0.12]" : "bg-white border-gray-200 hover:border-gray-300 shadow-sm"
          }`}
        >
  ...
  return (
    ...
    >
      <h2 className={`text-lg font-bold mb-4 ${dark ? "text-white" : "text-gray-900"}`}>
        Quick Start What do you want to achieve?
      </h2>
      ...
      >
        ...
        <span className={`text-sm ${dark ? "text-gray-500" : "text-gray-500"}`}>
          Type your own goal… <span className="text-indigo-400 font-medium">e.g. "Learn Japanese in 6 months"</span>
        </span>
      </motion.div>
    </motion.div>
  );
...
export default function Dashboard() {
  ...
  return (
    <div className={`flex h-screen overflow-hidden ${dark ? "bg-[#0d0d0d] text-white" : "bg-gray-100 text-gray-900"}`}>
      <Sidebar dark={dark} collapsed={collapsed} setCollapsed={setCollapsed} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} />
