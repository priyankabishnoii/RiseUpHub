...
function SectionCard({ section, content, isLoggedIn, defaultOpen }) {
  ...
  return (
    <div className={`rounded-2xl border overflow-hidden transition-all ${
      isLocked ? "border-white/[0.05] bg-white/[0.01] opacity-75" : "border-white/[0.07] bg-white/[0.03]"
    }`}>
      <button
        onClick={() => !isLocked && setOpen(!open)}
        className={`w-full flex items-center gap-3 p-5 text-left ${isLocked ? "cursor-not-allowed" : "hover:bg-white/[0.02]"} transition-colors`}
      >
        <div className={`w-9 h-9 rounded-xl ${isLocked ? "bg-white/10" : section.color} flex items-center justify-center flex-shrink-0`}>
          {isLocked ? <Lock className="w-4 h-4 text-gray-500" /> : <section.icon className="w-4 h-4 text-white" />}
        </div>
        <div className="flex-1 text-left">
          <div className={`font-bold text-base ${isLocked ? "text-gray-600" : "text-white"}`}>{section.title}</div>
          {isLocked && <div className="text-xs text-gray-600 mt-0.5">Sign in free to unlock</div>}
        </div>
        {!isLocked && (open ? <ChevronUp className="w-4 h-4 text-gray-500 flex-shrink-0" /> : <ChevronDown className="w-4 h-4 text-gray-500 flex-shrink-0" />)}
        {isLocked && <Lock className="w-4 h-4 text-gray-700 flex-shrink-0" />}
      </button>
    ...
    async function load() {
      try {
        ...
        const { data, error: err } = await supabase
          .from("roadmap_library").select("*").eq("id", id).single();
        if (err || !data) { setError("Roadmap not found."); }
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="sticky top-0 z-20 border-b border-white/[0.06] bg-[#0a0a0a]/95 backdrop-blur-xl">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 min-w-0">
            <button onClick={() => router.push("/browse")} className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors text-sm font-medium flex-shrink-0">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {user ? (
              <a href="/generate" className="flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold transition-all hover:scale-105">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black tracking-tight mb-5 leading-tight">{roadmap?.goal}</h1>
          ...
          {!user && (
            <div className="p-4 rounded-2xl border border-indigo-500/30 bg-indigo-600/8 flex flex-col sm:flex-row items-start sm:items-center gap-4">
              <div className="flex items-center gap-2 flex-1">
                <Lock className="w-5 h-5 text-indigo-400 flex-shrink-0" />
                <div>
                  <div className="text-sm font-bold text-white">Some sections are locked</div>
                  <div className="text-xs text-gray-400">Sign in free to unlock weekly schedule, projects, interview prep and more</div>
                </div>
              </div>
              <a href="/login" className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold transition-all whitespace-nowrap">
            ...
            <div className="p-4 rounded-2xl border border-violet-500/20 bg-violet-600/5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
              <div className="flex items-center gap-2 flex-1">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-400">This is a general roadmap. Get one tailored to your exact level and schedule.</div>
                </div>
              </div>
              <a href="/generate" className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-violet-600 hover:bg-violet-500 text-white text-sm font-bold transition-all whitespace-nowrap">
                <Sparkles className="w-4 h-4" />
                Personalize with AI
              </a>
            </div>
        ...
        {/* Bottom CTA */}
        <div className="mt-10 p-8 rounded-3xl border border-indigo-500/20 bg-indigo-600/5 text-center">
          <Sparkles className="w-10 h-10 text-indigo-400 mx-auto mb-3" />
          <h3 className="text-2xl font-black mb-2">{user ? "Make This Roadmap Yours" : "Unlock Everything"}</h3>
          <p className="text-gray-400 text-sm mb-6 max-w-sm mx-auto">
            {user ? "Generate a 100% personalized version with your exact schedule and goals." : "Sign in free to unlock all sections and generate a personalized AI roadmap."}
          </p>
          <a href={user ? "/generate" : "/login"}
            ...
            {user ? <><Sparkles className="w-5 h-5" /> Generate My Personalized Roadmap</> : <><ArrowRight className="w-5 h-5" /> Sign in Free</>}
          </a>
        </div>
      </div>
    </div>
